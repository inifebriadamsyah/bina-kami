      ...
      {[...new Array(2)].fill(0).map((_, index) => (
        <React.Fragment key={index}>
          {props.testimonials.map(({ text, imageSrc, name, username, alt }) => (
            <div className="p-10 border border-[#F1F1F1] rounded-3xl shadow-[0_7px_14px_#EAEAEA] max-w-xs w-full" key={alt}>
              <div className="text-start font-normal text-gray-700">{text}</div>
              <div className="flex items-center gap-2 mt-5">
                ...
                <div className="flex flex-col text-start">
                  <div className="font-bold tracking-wide leading-5 text-[#2d721f] uppercase">
                    {name}
                  </div>
                  <div className="leading-5 text-xs font-semibold text-gray-500">{username}</div>
                </div>
              </div>
            </div>
          ))}
